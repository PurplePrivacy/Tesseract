...
export function computePrioritiesForFile(file) {
    ...
    const prioritized = candidates
        ...
        return {
            metric,
            label: entry?.label,
            priorityScore: Number((entry.severity * weights[metric] * changeRisk * hotspot).toFixed(3)),
            insight: INSIGHT_MESSAGES[metric]?.[entry.label] ??
    ...
    const level = filePriority >= 0.35 ? "HIGH" : filePriority >= 0.2 ? "MEDIUM" : "LOW";
